...
const isOwner = async (req, res, next) => {
    try {
        ...
        if (!currentUserId) {
            return (0, response_1.sendResponse)(res, 400, "cant delete");
        }
        if (currentUserId.toString() !== id) {
            return (0, response_1.sendResponse)(res, 400, "You can't delete others accounts");
        }
        next();
    }
    catch (error) {
...
const isAuthenticated = async (req, res, next) => {
    try {
        ...
        if (!sessionToken) {
            return (0, response_1.sendResponse)(res, 400, "Session expired");
        }
        const existingUser = await (0, users_1.getUserBySessionToken)(sessionToken);
        if (!existingUser) {
            return (0, response_1.sendResponse)(res, 400, "Invalid token");
        }
